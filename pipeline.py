...
LUA_SHA1 = get_hash(os.path.join(CWD, 'galerie-cz.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        wget_args = [
            ...
            '--warc-header', 'galerie-cz-dld-script-version: ' + VERSION,
            '--warc-header', ItemInterpolation('galerie-cz-item: %(item_name)s'),
        ...
        if item_type == 'galerie':
            wget_args.extend(['--warc-header', 'galerie-cz-gallery: ' + item_value])
            wget_args.append('http://{}.galerie.cz/'.format(item_value))
        else:
...
pipeline = Pipeline(
    ...
    GetItemFromTracker('http://%s/%s' % (TRACKER_HOST, TRACKER_ID), downloader,
        VERSION),
    PrepareDirectories(warc_prefix='galerie-cz'),
    WgetDownload(
        WgetArgs(),
        max_tries=2,
